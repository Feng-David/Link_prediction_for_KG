...
def jc_predict(G: object) -> dict:
        ...
        for right_element in right_set:
            ...
            if not (left_element, right_element) in G.edges:
                try:
                    ...
                    if jaccard_sim[left_element][right_element] > 0.1:
    ...
    for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
        res.append((k[0],k[1],jaccard_sim[k[0]][k[1]]))
    df = pd.DataFrame(res, columns=['left_element','right_element','Probability'])
    df.to_csv('predictions/jaccard.csv',index=None,encoding='ANSI')
...
def Adamic_adar_predict(G: object) -> dict:
    """
    Compute the Jaccard-Needham dissimilarity between two 1-D arrays.
    :param G: Networkx bipartite graph
        ...
        for right_element in right_set:
            ...
            if not (left_element, right_element) in G.edges:
                try:
                    ...
                    if aa_sim[left_element][right_element] > 1:
    ...
    res = []
    for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
        res.append((k[0],k[1],aa_sim[k[0]][k[1]]))
    df = pd.DataFrame(res, columns=['left_element','right_element','Probability'])
    df.to_csv('predictions/adamic_adar.csv',index=None,encoding='ANSI')
    # for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
    #     # print(k[0],v)
    #     out.write(str((k[0], k[1])))
    #     out.write(",")
...
def common_neighbor_predict(G: object) -> dict:
    """
    ...
    for left_element in left_set:
        ...
        for right_element in right_set:
            ...
            if not (left_element, right_element) in G.edges:
                ...
                if cn_sim[left_element][right_element] > 3:
                    dictionary.update({(left_element, right_element): cn_sim[left_element][right_element]})
    res = []
    for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
        res.append((k[0],k[1],cn_sim[k[0]][k[1]]))
    df = pd.DataFrame(res, columns=['left_element','right_element','Probability'])
    df.to_csv('predictions/common_neighbor.csv',index=None,encoding='ANSI')
    # for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
    #     # print(k[0],v)
    #     out.write(str((k[0], k[1])))
    #     out.write(",")
...
def preferential_attachment_predict(G: object) -> dict:
    """
    ...
    for left_element in left_set:
        ...
        for right_element in right_set:
            ...
            if not (left_element, right_element) in G.edges:
                ...
                if pa_sim[left_element][right_element] > 100:
                    dictionary.update({(left_element, right_element): pa_sim[left_element][right_element]})
    res = []
    for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
        res.append((k[0],k[1],pa_sim[k[0]][k[1]]))
    df = pd.DataFrame(res, columns=['left_element','right_element','Probability'])
    df.to_csv('predictions/preferential_attachment.csv',index=None,encoding='ANSI')
    # for k, v in sorted(dictionary.items(), key=itemgetter(1), reverse=True):
    #     # print(k[0],v)
    #     out.write(str((k[0], k[1])))
    #     out.write(",")
...
def katz_predict(G: object) -> dict:
    ...
    out = open('./predictions/katz.csv', 'w')
    # outN = open('./predictions/preferential_attachment_with_name.csv', 'w')
    hop2s = dict()
    neighbors = dict()
